...
def resolve_create_mention(_, info, batch_id, applicant_id, comment_id, mentioned_id):
    ...
    if(current_user):
        try:
            user_doc, user_details =  get_user_document(info, mentioned_id)
        ...
        mention_doc.set({'mentionData': mention_data, 'mentioner': mentioner_user, 'createdAt': mention_date, 'read': bool('false')})
